...
**Supabase CLI access token** (enables Claude to deploy functions/migrations without switching CLI accounts):

1. Log in to the client's Supabase account → `Account → Access tokens → Generate new token`
2. Copy `execution/frontend/app/.env.example` → `execution/frontend/app/.env`
3. Paste the token as `SUPABASE_ACCESS_TOKEN=<token>`

> `.env` is gitignored. The `settings.local.json` supabase permissions source this file automatically,
> so Claude can run `supabase functions deploy` and `supabase db push` without logging out of your main account.
